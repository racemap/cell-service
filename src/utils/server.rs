...
async fn get_cells(query: GetCellsQuery) -> Result<impl warp::Reply, warp::Rejection> {
    let connection = &mut establish_connection();
...
pub async fn start_server(shutdown_receiver: Receiver<()>) -> Promise<()> {
    ...
    let get_cells = warp::path!("cells")
        .and(warp::query::<GetCellsQuery>())
        .and_then(|query| async move { get_cells(query).await });
...
mod tests {
    ...
    mod get_cells_query {
        ...
        #[test]
        fn test_deserialize_from_query_string() {
            let query_string = "mcc=262&mnc=1&min_lat=52.0&max_lat=53.0&min_lon=13.0&max_lon=14.0&limit=100";
